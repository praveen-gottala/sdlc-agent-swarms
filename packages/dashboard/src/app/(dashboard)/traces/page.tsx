...
/** Task IDs are now fetched dynamically from /api/traces. */
...
function mapTrace(apiTrace: Record<string, unknown>): TraceEntry {
  ...
  const mappedSteps: TraceTimelineStep[] = steps.map((s) => {
    const hasLlmMeta = !!(s as Record<string, unknown>).llmMeta;
    const status = (s as Record<string, unknown>).status as string | undefined;
    let stepType: 'stage' | 'llm-call' | 'log' = 'stage';
    if (hasLlmMeta) stepType = 'llm-call';
    else if (status === 'log') stepType = 'log';
...
export default function TracesPage() {
  ...
  useEffect(() => {
    // Fetch task IDs dynamically, then fetch each trace
    fetch('/api/traces')
      .then(res => res.json())
      .then(json => {
          ...
          taskIds.map(taskId =>
            fetch(`/api/traces/${taskId}`)
              ...
              .then(traceJson => {
                ...
                // Skip traces with no real data
                if (trace.status === 'not_found') return null;
  ...
  if (traces.length === 0) return <div className="flex items-center justify-center h-64 text-text-muted">No traces yet</div>;

  const selected = traces.find((t) => t.id === selectedId) ?? traces[0];
  const filteredSteps = showDebugLogs
    ? selected.steps
    : selected.steps.filter((s) => s.stepType !== 'log');
    ...
    <div className="flex flex-col h-full min-h-0 gap-5">
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="flex-1 min-w-0 overflow-y-auto flex flex-col gap-5">
          ...
          />
          <TraceTimeline steps={filteredSteps} />
        </div>
      </div>
    </div>
  );
