...
- [x] **Gate 6a:** Spine structural quality ≥ standalone design pipeline baseline on same screen (parent M4 eval criterion). **PASSED (2026-05-18).** Greenfield: SUCCESS, 1497.6s, $6.24. Brownfield: SUCCESS, 874.5s, $3.96. Failure rate: 0%. Results at `packages/eval/results/m4/spine-eval-results.json`.
- [x] **Gate 6b:** Re-run design-info matrix subset — confirm ADR-057 routing in production path matches M3.6 recommendation (mean fidelity within ±0.15 per task type). **PASSED (2026-05-18).** Config A NEW: 2.08 (within [1.74, 2.04] — marginal improvement). Config E MODIFY: 2.50 (within [2.41, 2.71]). Results at `packages/eval/results/m4/regression-results.md`.
