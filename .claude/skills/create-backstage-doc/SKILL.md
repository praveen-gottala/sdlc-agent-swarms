---
name: create-backstage-doc
description: |
  Create or revise an internal CHIP backstage documentation page in `docs/`. Use when the user asks to write, rewrite, or fix a concept page, tutorial, guide, architecture page, or status page — especially when current content reads like a blog post, contains internal engineering metrics, mixes audiences, or fails the "could a competitor have written this?" test. Invoke as `/create-backstage-doc <type> <topic>` where type is one of: concept, tutorial, guide, architecture, status.
---

# /create-backstage-doc

Produces backstage docs for CHIP — an SDLC framework being demoed to leadership and rolled out to internal employees. The skill enforces specificity, audience awareness, "show the reader the mental model" before showing the API surface, and zero aspirational tense. Reference standard is LangGraph + Prisma for structural patterns, react.dev for tutorial pacing, Linear for status sparseness.

## When to use

- User asks to write a new page in `docs/`
- User asks to "fix" or "redo" an existing page
- User flags a page as generic, marketing-y, blog-post tone, or jargon-dense for the audience
- User asks to add a missing concept, tutorial, guide, architecture, or status page
...
- **`/create-backstage-doc`** writes and revises **one page at a time**. It optimizes for the quality of that page: specificity, mental-model-first opening, swap-test compliance, jargon control.
...
- After running `/create-backstage-doc` on a page, `/verify-docs` should pass on that page (or be expected to fail only on triggers unrelated to content quality, like CLI sync or vision-layer drift).
- When `/verify-docs` flags a page as stale, the fix is to re-run `/create-backstage-doc <type> <topic>` on it. The pre-edit staleness pass in Step 1 will catch the broken paths.

If you find yourself wanting to audit the whole site from inside this skill, stop — that's `/verify-docs`'s job.

## Inputs

`/create-backstage-doc <type> <topic>`
...
- Parse `$ARGUMENTS` into `type` and `topic`.
