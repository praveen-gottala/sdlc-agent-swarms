...
5. CHIP UX Overhaul — Rebrand from AgentForge to CHIP (Crafted Human Intelligence Platform). Phase 1 COMPLETE (2026-04-28). Phase 2 COMPLETE (2026-04-29, committed `ca5df49`): Mantine AppShell, Tabler Icons, grouped sidebar, draggable resize, dev server speed fix, navigation consolidation, E2E selector fixes. Phase 4.1 COMPLETE (2026-04-29, committed `ae0e8ba`): Home page redesigned as state-aware landing pad (pipeline spine, attention items, quick actions). CHIP logo moved to header, project identity in sidebar. Next: Phase 4.0 (Pipeline → Runs page — CRITICAL, wrong 5-phase model). See `docs/plans/active/chip-ux-overhaul/execution-plan.md`
...
**Last session (2026-04-29):** Phase 2 committed (`ca5df49`) + Phase 4.1 Home page committed (`ae0e8ba`). Home page redesigned 3 times: stat cards → data dashboard → state-aware landing pad. Key learnings: no AI aesthetic (gradient rings, glass cards = tells), home is a routing pad not a mission control (Pipeline/Runs is the mission control per vision Layer 14), `window.location.reload()` for project switching (not `router.refresh()`). Layout: CHIP logo in header (42px, dark/creme per theme), project identity in sidebar (initials + name). Next: Phase 4.0 Pipeline → Runs page.
