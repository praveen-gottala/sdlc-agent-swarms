...
export interface GeneratedPage {
  ...
  readonly navigates_to?: readonly { readonly target: string; readonly trigger: string }[];
...
export function parseAppSpecResponse(raw: string): GeneratedAppSpec | null {
  try {
    ...
    // Validate navigation targets reference valid page IDs
    const validScreenTypes = new Set(['page', 'modal', 'drawer', 'sheet']);
    const validPageIds = new Set(validPages.map(p => p.id));
    const pagesWithValidNav = validPages.map(p => {
      const screenType = p.screen_type && validScreenTypes.has(p.screen_type)
        ? p.screen_type
        : 'page' as const;

      let result: GeneratedPage = { ...p, screen_type: screenType };

      if (p.navigates_to) {
        const validNav = p.navigates_to.filter(
          (n: { target?: string; trigger?: string }) => n.target && n.trigger && validPageIds.has(n.target),
        );
        if (validNav.length > 0) {
          result = { ...result, navigates_to: validNav };
        }
      }
