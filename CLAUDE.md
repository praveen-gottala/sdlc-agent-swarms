...
2. CHIP's Next Steps — M0 COMPLETE (2026-05-04), M1 COMPLETE (2026-05-14), M2 COMPLETE (2026-05-14), M3 COMPLETE (2026-05-15), M3.5 COMPLETE (2026-05-15). M3.5 delivers: R9 brownfield design delta research brief (664 lines), CashPulse brownfield fixture (`packages/eval/src/scenarios/cashpulse-brownfield.yaml`), hybrid `DesignSpecDelta` schema, `AffectedScreen` impact classification, slice-aware `ContextRefKind` extension, verified token budget (~25-35K of 76K ceiling). Script: `scripts/run-clarifier-cashpulse-brownfield.ts`. Verification review: `docs/research/briefs/R9-brownfield-design-delta-review.md`. Next: M3.6 (Design Info Value Eval), then M4 (Full Spine). See `docs/plans/active/chips-next-steps/execution-plan.md`
...
**Last session (2026-05-15):** M3.5 COMPLETE. Clarifier evolution mode run on CashPulse brownfield ("Add recurring transactions"): 258s, 6 screens, 3 entities, 8 features, 4/4 schemas validated. Fixture: `packages/eval/src/scenarios/cashpulse-brownfield.yaml`. R9 research brief (664 lines) at `docs/research/briefs/R9-brownfield-design-delta.md`: 8 recommendations (screen-level impact classification, hybrid delta schema, single-call-per-screen strategy, ContextRefKind extension, configurable slice strategy for M3.6). Verification review confirmed 13/15 citations accurate, 2 drift corrected. Script: `scripts/run-clarifier-cashpulse-brownfield.ts`. Next: M3.6 (Design Info Value Eval), then M4 (Full Spine).
