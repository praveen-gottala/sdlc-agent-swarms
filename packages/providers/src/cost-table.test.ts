...
describe('cost-table', () => {
  ...
  describe('getModelCost', () => {
    it('returns known model costs', () => {
      const cost = getModelCost('claude-sonnet-4');
  ...
  describe('calculateCost', () => {
    it('calculates cost correctly for claude-sonnet-4', () => {
      const result = calculateCost('claude-sonnet-4', 1_000_000, 1_000_000);
  ...
  describe('setCostOverrides', () => {
    it('overrides existing model costs', () => {
      setCostOverrides({ 'claude-sonnet-4': { input: 5.0, output: 20.0 } });
      expect(getModelCost('claude-sonnet-4')).toEqual({ input: 5.0, output: 20.0 });
    });
