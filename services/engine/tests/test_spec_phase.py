"""Tests for the spec phase graph via the server API."""
...
@pytest.mark.asyncio
async def test_spec_happy_path(client: AsyncClient, tmp_project: Path) -> None:
    """Spec writing → decomposition → review interrupt → approve."""
...
@pytest.mark.asyncio
async def test_spec_changes_requested(
    client: AsyncClient, tmp_project: Path
) -> None:
    """Changes requested sets task status accordingly."""
    resp = await client.post(
        "/phase/start",
        json={"phase": "spec", "project_root": str(tmp_project)},
    )
    ...
    tasks = load_tasks(tmp_project)
    task_1 = next(t for t in tasks.tasks if t.id == "task-1")
    assert task_1.status == "changes_requested"
